...
class MarmIntegration:
    ...
    def _setup_storage(self):
        ...
        try:
            ...
            # Automatically restore existing sessions from storage
            self._restore_sessions_from_storage()
    ...
    def _restore_sessions_from_storage(self):
        """Restore sessions from storage file to memory."""
        try:
            import json
            ...
            if storage_path.exists():
                ...
                sessions_json = storage_data.get('marm-sessions-v1', '{}')
                if sessions_json:
                    sessions_data = json.loads(sessions_json)
                    
                    # Update the sessions in the already imported implementation module
                    # The sessions variable is already imported at the top level
                    sessions.update(sessions_data)
                    
                    print(f"Restored {len(sessions_data)} sessions from storage")
        except Exception as e:
            print(f"Warning: Failed to restore sessions from storage: {e}")
