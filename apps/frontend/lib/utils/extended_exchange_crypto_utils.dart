...
class ExtendedExchangeCryptoUtils {
  ...
  static Uint8List _hashStruct(String primaryType, Map<String, dynamic> data, Map<String, dynamic> types) {
    final typeHash = _hashType(primaryType, types);
  ...
  static List<String> _findTypeDependencies(String primaryType, Map<String, dynamic> types, [Set<String>? found]) {
    found ??= <String>{};
  ...
  static Uint8List _encodeData(String primaryType, Map<String, dynamic> data, Map<String, dynamic> types) {
    final fields = types[primaryType] as List<dynamic>;
    final encoded = <int>[];
  ...
  static List<int> _encodeValue(String type, dynamic value, Map<String, dynamic> types) {
    if (type == 'string') {
      ...
      if (addressStr.startsWith('0x')) {
        return _hexToBytes(addressStr.substring(2).padLeft(40, '0')).padLeft(32, 0);
      } else {
  ...
  static String _signWithEthereumKey(Uint8List messageHash, String privateKey) {
    ...
    final messageHashBigInt = BigInt.parse(messageHash.map((b) => b.toRadixString(16).padLeft(2, '0')).join(), radix: 16);
  ...
  static BigInt _secp256k1Order() {
    return BigInt.parse('fffffffffffffffffffffffffffffffebaaedce6af48a03bbfd25e8cd0364141', radix: 16);
  }
  ...
  static Map<String, BigInt> generateStarkNetSignature({
    ...
    final starkFieldPrime = BigInt.parse('3618502788666131213697322783095070105623107215331596699973092056135872020481');
  ...
  static Future<bool> testCryptoUtils() async {
    ...
    try {
      ...
      final signature = signEIP712Message(
        message: keyStruct,
        privateKey: '0x50c8e358cc974aaaa6e460641e53f78bdc550fd372984aa78ef8fd27c751e6f4',
      );
      ...
      print('✅ StarkNet signature generated: r=${starkSig['r']?.toRadixString(16).substring(0, 10)}...');
