...
import '../services/extended_exchange_l2_key_service.dart';
import '../utils/extended_exchange_crypto_utils.dart';
...
class ExtendedExchangeOnboardingService {
  ...
  static Future<OnboardingPayload> _createOnboardingPayload({
    ...
    final timeString = now.toIso8601String().replaceAll('T', 'T').replaceAll('Z', 'Z');
    ...
    final registrationStruct = ExtendedExchangeCryptoUtils.createAccountRegistrationStruct(
      accountIndex: 0,
  ...
  static Future<OnboardedClientModel> _submitOnboardingRequest(OnboardingPayload payload) async {
    print('📡 Submitting onboarding request to Extended Exchange...');
    
    final url = '${ExtendedExchangeConfig.STARKNET_SEPOLIA_ONBOARDING}/auth/onboard';
    print('   URL: $url');
    ...
    try {
      ...
      if (response.statusCode == 200 || response.statusCode == 201) {
        ...
        if (responseData['data'] != null) {
          final onboardedClient = OnboardedClientModel.fromJson(responseData['data']);
  ...
  static Future<String> _createTradingApiKey({
    ...
    final description = 'AstraTrade Trading Key - ${DateTime.now().toIso8601String()}';
    
    // Create authentication headers
    final now = DateTime.now().toUtc();
    final authTimeString = now.toIso8601String().replaceAll('T', 'T').replaceAll('Z', 'Z');
    ...
    final url = '${ExtendedExchangeConfig.STARKNET_SEPOLIA_ONBOARDING}$requestPath';
    ...
    try {
      final response = await http.post(
        ...
        body: json.encode(ApiKeyRequestModel(description: description).toJson()),
      ).timeout(_httpTimeout);
      ...
      if (response.statusCode == 200 || response.statusCode == 201) {
        ...
        if (responseData['data'] != null) {
          final apiKeyResponse = ApiKeyResponseModel.fromJson(responseData['data']);
